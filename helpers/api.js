...
import {ACCESS_TOKEN as PAGE_ACCESS_TOKEN} from '../config';
...
const callAPI = (endPoint, messageDataArray, queryParams = {}, retries = 5) => {
  ...
  if (!endPoint) {
    console.error('[ERROR]: callAPI requires you to specify an endpoint.');
    return;
  }

  if (!PAGE_ACCESS_TOKEN) {
    console.error('[ERROR]: callAPI requires you to specify a page access token.');
    return;
  }
